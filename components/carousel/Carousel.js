...
class Carousel {
  ...
  #synchronize () {
    for (let observation of this.hasIntersected) {
      ...
      const dot = this.elements.pagination.children[
  ...
  #unlisten () {
    for (let item of this.elements.snaps) this.carousel_observer.unobserve(item)
  ...
  #createControl (btnType) {
    ...
    let userFacingText = `${btnType.charAt(0).toUpperCase() +
      btnType.slice(1)} Item`

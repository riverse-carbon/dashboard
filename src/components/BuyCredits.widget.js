  ...
  return (
    <div className={`${styles['widget-wrapper']} ${styles['text-bold']}`}>
      {showConfirmationWidget ? (
        <ConfirmationWidget
          transactionInfo={formInputs}
          totalPrice={totalPrice}
          cancelPurchase={() => setShowConfirmationWidget(false)}
          resetInputs={resetState}
        />
      ) : (
        <>
          ...
          <div className={styles.body}>
            ...
            />
            <div className={`${styles['tab-content-wrapper']} flow-spacer spacer-xs`}>
              ...
              <button className={`${styles.submit}`} type='submit' form='buy-credits-form'>
                Start transaction
              </button>
              <p className={styles['purchase-total-wrapper']}>
                ...
                <span className={styles['text-normal']}>{totalPrice}€</span>
              </p>
            </div>
          </div>
        </>
...
const VintageYearSelect = ({ years, updateInput }) => {
  ...
  return (
    <select className={styles['vintage-year-select']} onChange={handleChange}>
      {options}
    </select>
  );
};
...
const ConfirmationWidget = ({ transactionInfo, totalPrice, cancelPurchase, resetInputs }) => {
  ...
  return (
    <div className={`${styles['text-bold']} flow-spacer ${styles['confirmation-widget']}`}>
      {transaction ? (
        <TransactionResult success={transaction.success} resetInputs={resetState} />
      ) : (
        <>
          <h2>Your order</h2>
          <p>
          ...
          <p>
            Developer:<span>{name}</span>
          </p>
          <p>
            CO2 type:<span>{creditType}</span>
          </p>
          <p>
            Carbon credits selected:<span>{creditsPurchased}</span>
          </p>
          <p>
            Price:<span>{price} €</span>
          </p>
          <p>
            Vintage year:<span>{vintageYear}</span>
          </p>
          <p className={styles['confirmation--total']}>
            Total
            <span
              className={styles['confirmation--price-with-icon']}
              title='Riverse service fee: 10% of the carbon credit price is collected by Riverse to continue its work on eco-sustainability projects.'>
              {totalPrice} €<InformationSVG />
            </span>
          </p>
          <div className={styles['confirmation--buttons-wrapper']}>
            <button className={styles['confirmation--submit']} onClick={handleCreditPurchase}>
              Start transaction
            </button>
            <button className={styles['confirmation--cancel']} onClick={cancelPurchase}>
              Cancel transaction
            </button>
          </div>
        </>
  ...
  return (
    <>
      <h2 className='text-center'>
        {success ? 'Successful transaction' : 'Something went wrong, please contact us if the problem persists'}
      </h2>
      <button onClick={handleButtonClick} className={styles['transaction--button']}>
        Ok
      </button>
    </>
  );

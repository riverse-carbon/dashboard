...
const TransactionDetails = ({ data }: TransactionDetailsProps): JSX.Element => {
  ...
  return (
    <>
      ...
      <div>
        <div className='grid grid-cols-2 px-5 py-2.5 bg-bg-secondary'>
        ...
        <div className='grid grid-cols-2 px-5 py-2.5 bg-bg-secondary'>
          ...
          <span className='font-medium'>{dayjs(purchaseDate).format('D MMMM YYYY')}</span>
        </div>
        <div className='grid grid-cols-2 px-5 py-2.5'>
          <span className='flex gap-2.5 items-center'>
            <CalendarApprovedSVG /> Date of validation:
          </span>
          <span className='font-medium'>{dayjs(validationDate).format('D MMMM YYYY')}</span>
        </div>
      </div>
    </>
  );

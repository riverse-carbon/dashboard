...
const createNewUser = async (req, res) => {
  ...
  var currentUserFromDB = await getById(base, usersTable, currentUserId)

  if (!currentUserEmail === currentUserFromDB.fields.email) {
    res.status(403)
    throw new Error('You do not have permissions to perform this operation')
  }

  var createUserPermissionsByRole = []
  switch(currentUserFromDB.fields.role) {
    case 'admin':
...
// users table
const response = await base('tblG56b6iiigWe8kI').create([{
  'fields': {
    'surname': 'John',
    'name': 'Doe',
    'email': userToAdd.email,
    "org": [currentUserFromDB.fields.org[0]],
    "role": userToAdd.role
  }
}])
.then((records) => ({success: !!records[0].id, email: records[0].fields.email}))
